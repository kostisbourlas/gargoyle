...
class GargoyleModule(nexus.NexusModule):
    ...
    def get_urls(self):
        return [
            ...
            re_path(
                r"^conditions/add/$",
                self.as_view(self.add_condition),
                name="add-condition",
            ),
            re_path(
                r"^conditions/remove/$",
                self.as_view(self.remove_condition),
    ...
    def render_on_dashboard(self, request):
        ...
        switches = list(
            Switch.objects.exclude(status=DISABLED).order_by("date_created")[:5]
        )
    ...
    def add(self, request):
        ...
        if len(key) > 64:
            raise GargoyleException(
                "Key must be less than or equal to 64 characters in length"
            )
        ...
        if len(label) > 64:
            raise GargoyleException(
                "Name must be less than or equal to 64 characters in length"
            )
        ...
        logger.info(
            ...
            ", ".join(
                ...
                for k in sorted(("key", "label", "description"))
            ),
        )
    ...
    def update(self, request):
        ...
        if changes:
            ...
            logger.info(
                ...
                ", ".join(
                    "%s=%r->%r" % (k, v[0], v[1]) for k, v in sorted(changes.items())
                ),
            )
    ...
    def status(self, request):
        ...
        if switch.status != status:
            ...
            logger.info(
                "Switch %r updated (status=%%s->%%s)" % switch.key,
                old_status_label,
                switch.get_status_display(),
            )
    ...
    def add_condition(self, request):
        ...
        if not switch.can_add_condition(namespace, field_name, value):
            raise GargoyleException(
                "Conditions cannot conflict with or duplicate existing conditions"
            )
    ...
    def remove_condition(self, request):
        ...
        logger.info(
            "Condition removed from %r (%r, %s=%r)"
